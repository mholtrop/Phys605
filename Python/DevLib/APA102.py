...
# However, if you want this to run glitch free at high data rates and full brightness,
# 5V power and logic are recommended.
#
try:
...
class APA102(np.ndarray):
    ...
    :param input_array  - Standard np.ndarray for initialization,
    ...
    def __new__(cls, input_array, cs=None, clk=None, mosi=None):
        ...
        if isinstance(input_array, np.ndarray):
            ...
            obj = np.asarray(np.zeros(shape=input_array, dtype=np.int32)).view(cls)
    ...
    def __str__(self):
        ...
        if self.ndim == 1:
            for x in self:
                out += "({:03x},{:03x},{:03x},{:03x}) ".format((x >> 24) & 0xFF, x & 0xFF, (x >> 8) & 0xFF,
                                                               (x >> 16) & 0xFF)
            out = out + "]"
            return out
        if self.ndim == 2:
            for x in self:
                out = out+"["
                for y in x:
                    out += "({:03x},{:03x},{:03x},{:03x}) ".format((y >> 24) & 0xFF, y & 0xFF, (y >> 8) & 0xFF,
                                                                   (y >> 16) & 0xFF)
                out = out+"]\n"
            out = out + "]"
            return out
...
#        self._dev.close()
    ...
    def show(self):
        ...
        self._dev.writebytes([int(i) for x in self.ravel() for i in [(0xe0 | ((x >> 24) & 0x1F)), (x >> 16) & 0xFF,
                                                                     (x >> 8) & 0xFF, x & 0xFF]])
